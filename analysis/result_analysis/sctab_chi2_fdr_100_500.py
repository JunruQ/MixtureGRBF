...
# 按照 subtype 和 cell type order 排序
# 设定为有序分类
chi_df["cell_type"] = pd.Categorical(
    chi_df["cell_type"],
...
chi_df = chi_df.sort_values(
    by=["subtype", "cell_type"]
).reset_index(drop=True)
chi_df.to_csv(exp_path + f'sctab_chi_square_{top_n}.csv', index=False)
...
fig = plt.figure(figsize=(16/2.54, (2 * n_subtypes)))
...
plt.savefig(exp_path + f'cell_type_chi2_fdr_scatter_{top_n}.png', bbox_inches='tight', dpi=500)

...
# 读取数据
data_all <- read.csv('output/result_analysis/ukb_MixtureGRBF_cv_nsubtype_biom17/5_subtypes/t_stats_protein_49_64.csv')

for(age in c(49, 64)) {
  data <- data_all[data_all$Age == age, ]
  # 将 SYMBOL 转换为 ENTREZID
  gene_entrezid <- bitr(geneID = data$Biomarker, 
                        fromType = "SYMBOL", 
                        toType = "ENTREZID",
                        OrgDb = "org.Hs.eg.db")
  ...
  # 保存csv文件
  write.csv(final_data, glue("output/result_analysis/ukb_MixtureGRBF_cv_nsubtype_biom17/5_subtypes/gsea_results_{age}.csv"), row.names = FALSE)

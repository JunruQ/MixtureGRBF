...
# # ANOVA test to select top N significant factors
# anova_results = []
# for ind in continuous_indicators:
#     df_anova = df[[ind, 'subtype']].dropna()
#     df_anova['subtype'] = df_anova['subtype'].astype(str)
#     formula = f'Q("{ind}") ~ C(subtype)'
#     model = ols(formula, data=df_anova).fit()
#     anova_table = anova_lm(model, typ=2)
#     pval = anova_table['PR(>F)']['C(subtype)']
#     anova_results.append({'Indicator': ind, 'ANOVA_p': pval})

# anova_df = pd.DataFrame(anova_results)
# top_indicators = anova_df.nsmallest(N_TOP_FACTORS, 'ANOVA_p')['Indicator'].tolist()
...
# Create figure with subplots (one per subcategory)
fig, axes = plt.subplots(len(indicators_per_subcategory), 1,
                         figsize=(fig_width, fig_height),
                         sharex=True,
                         gridspec_kw={'height_ratios': height_ratios})

# Ensure axes is a list even if only one subplot
if len(indicators_per_subcategory) == 1:
    axes = [axes]
...
# Plot heatmap for each subcategory
for idx, subcategory in enumerate(indicators_per_subcategory.keys()):
    valid_indicators = [indicator_aliases.get(ind, ind) for ind in indicators_per_subcategory[subcategory]]
    if not valid_indicators:
        print(f"没有适用于 {subcategory} 的连续指标")
        axes[idx].set_visible(False)
        continue

    # Pivot table for heatmap
    pivot_signed = results_df[results_df['Indicator'].isin(valid_indicators)].pivot_table(
        values='Signed_-log10_p', index='Indicator', columns='Subtype', fill_value=0
    )
    ...
    axes[idx].set_title(f'{subcategory}')
    axes[idx].set_xlabel('')
    axes[idx].set_ylabel('')
    # Adjust y-tick label rotation for readability
    axes[idx].tick_params(axis='y', rotation=0)

# Adjust layout and save
plt.tight_layout()
output_path = f'{OUTPUT_DIR}/signed_logp_heatmaps_top_{N_TOP_FACTORS}_indicators_dynamic_height.png'

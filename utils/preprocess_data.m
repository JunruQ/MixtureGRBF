...
read_dir = './Tadpole/TADPOLE_input_pass.csv';
save_dir = './Tadpole/TADPOLE_input_pass_z.csv';

input = readmatrix(read_dir);
years = input(:,2);
diagnosis = input(:,3);
mat = input(:,10:end);
...
for i = 1:nbiom
    ...
    mean(mat_z_score(diagnosis==1,i))
end

%% save table
input_tab = readtable(read_dir);
for i = 1:nbiom
    input_tab{:,i+9} = mat_z_score(:,i);
end
writetable(input_tab,save_dir)